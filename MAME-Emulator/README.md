...
A MAME emulator with mobile compatibility designed for running in vanilla JavaScript pre-ECMAScript 2015 (no WebAssembly). Simply open the link below, click the red icon, and select a ROM file in `ZIP` format from your computer; it will be loaded and booted automatically. For Neo Geo games, you must have the `neogeo.zip` BIOS file and add its contents to the game's ROM `ZIP` file.
